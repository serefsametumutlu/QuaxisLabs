...
## FAZ 2 — Sektör altyapısı TAMAMLANDI (2026-08-11, commit `fdf242c`)
YOL_HARİTASI.md Faz 2 (sektör evren + sınıflandırma) bitti. `docs/spec/spec_sektor_evren.md` yazıldı, `kod-gelistirici` agent'ı ile uygulandı: Company modeline sektör alanları, `kap.py`/`sec_edgar.py` fetcher entegrasyonu, `scripts/refresh_universe.py` (rate-limit'e saygılı, checkpoint DB'nin kendisi — kaldığı yerden devam eder). 44 yeni test eklendi, toplam **1211/1211 yeşil**.

Sonuç — DB'de sektör etiketli iki evren:
- **BİST 643 şirket (TAM, tek istekle KAP'tan)** — Finans 142, Ana Metaller/Madencilik 130, Sanayi 83, Tüketici(Döngüsel) 80, GYO 57, Tüketici(Temel) 55, Kamu Hizmetleri 41, Teknoloji 40, İletişim 7, Sağlık 4, Enerji 4.
- **NASDAQ 4352 ticker keşfedildi, 1442'si SIC ile zenginleştirildi (~%33, tasarım gereği kademeli)** — Finans 646, Teknoloji 209, Sağlık 177, Sanayi 162, Tüketici(Döngüsel) 73, GYO 49, İletişim 40, Ana Metaller/Madencilik 31, Tüketici(Temel) 23, Kamu Hizmetleri 19, Enerji 13, (henüz işlenmedi) 2910. Kalan NASDAQ satırları `scripts/refresh_universe.py --market nasdaq --limit N` tekrar çalıştırılarak kademeli tamamlanır (90 gün tazelik penceresi).

Faz 3'teki n≥5 kuralı için: her iki piyasada her üst-sektörde n≥5 var — istisna BİST Sağlık/Enerji n=4 (Faz 3'te "yetersiz örneklem" uyarısı tetiklenecek).

Faz 2.4 canlı script çalıştırmasında bulunup düzeltilen 2 gerçek hata (spec/koddan değil, üretim çalıştırmasından çıktı):
1. BİST/NASDAQ sembol çakışması (LINK, LILAK) NASDAQ Adım 1'in tamamını (4347 satır) durduruyordu — artık satır bazında yakalanıp o ticker atlanıyor.
2. İlk düzeltme versiyonunda `session.rollback()` çakışma anına kadar commit edilmemiş ~3400 satırı da siliyordu (4347 bulunanın sadece 928'i kalıcı oluyordu) — rollback gereksizdi, kaldırıldı; artık 4352 satırın tamamı kalıcı.

**Sıradaki adım: FAZ 3a (Temel Analiz v2 spesifikasyonu)** — `temel-analiz-uzmani` agent'ı ile `docs/spec/` altına 4 mercek spec'i (Değer/Kalite/Büyüme/Güvenlik) + bileşik skor spec'i, `bilgi-bankasi/00_sentez.md` ve `temel-analiz-cercevesi` skill'i temel alınarak. Ayrı oturumda başlanmalı (YOL_HARİTASI.md Faz 3a promptu).

### 00_sentez.md üretildi (2026-08-11) — PROVİZYONEL (3/6 kitap)
`bilgi-bankasi/00_sentez.md` bu turda üretildi — Graham (01) + Buffett (02) + Damodaran (03)'ın TAM METNİ (üç dosyanın tamamı) okunarak: (1) kesişim noktaları (F/K, PD/DD, ROE, kazanç getirisi/güvenlik marjı, borç/özkaynak, faiz karşılama, çok-yıllı kazanç serisi, temettü, cari oran, ROIC, muhasebe kalitesi şüpheciliği), (2) çözülmeden bırakılan 9 çelişki (sabit eşik vs "ortalamaya göre ucuz" yanılgısı, Graham'ın kendi içindeki 1,33×/1,5× PD/DD farkı, borç/özkaynak tanım farkı, Cari Oran doğrudan çelişkisi [Graham ≥2,0 vs Buffett'ın "<1 sık ve normal" bulgusu], Buffett'ın sonsuza-dek-tut felsefesi vs Graham'ın sürekli-ucuzluk arayışı, temettü politikası görüş ayrılığı [Graham payout tercih eder vs Buffett geri-alım tercih eder], nakit biriktirme yorumu, büyüme hisselerine yaklaşım, matematiksel karmaşıklık felsefesi), (3) ~35 satırlık metrik→kitap/kod çapraz referans tablosu, (4) 13 maddelik öncelik sıralı konsolide veri eksikliği listesi + 10 düşük-maliyetli/yüksek-değerli kazanım listesi, (5) 4 merceğe kaba İLKE/FORMÜL/BAYRAK dağılımı ÜRETİLDİ. Dosyanın başında AÇIKÇA "DURUM: PROVİZYONEL (3/6 kitap)" notu var — **Fisher/Lynch/Schilit eklendiğinde §5 (mercek dağılımı, özellikle BÜYÜME ve GÜVENLİK sütunları) ve §2 (çelişkiler) MUTLAKA REVİZE EDİLMELİDİR.** BÜYÜME merceği şu an SADECE Damodaran'ın DCF-büyüme formülleri (temel büyüme=tutma×ROE) ve Graham'ın büyüme-şüpheciliğiyle temelleniyor — somut bileşen kümesi çok ince. GÜVENLİK merceği Graham'ın kırmızı bayrakları + Buffett'ın kaldıraç/likidite göstergeleri + Damodaran'ın Ch.17 sıkıntı-maliyeti/Merton çerçevesiyle ORTA düzeyde temelleniyor ama SİSTEMATİK muhasebe hilesi TESPİTİ (Schilit'in asıl konusu) tamamen eksik.

---

## FAZ 3a — Spec quant denetimi + veri tamlığı doğrulaması TAMAMLANDI (2026-08-11, commit `7d4675c`)

Faz 3a'da yazılan 5 taslak spec (spec_mercek_deger/kalite/buyume/guvenlik.md + spec_bilesik_skor.md, commit f3f63b9) bir sonraki oturumda İKİ paralel ajanla denetlendi:

**1) `quant-uzmani` ajanı — sayısal denetim + kalibrasyon:**
- `docs/spec/quant_denetim_01.md` yazıldı: 6 boyutta (sayısal kararlılık, birim tutarlılığı, monotonluk, çift sayma, küçük sektör davranışı, eksik veri yeniden dağıtımı) denetim → **5 KRİTİK + 5 YÜKSEK + 4 ORTA + 3 DÜŞÜK** bulgu.
- KRİTİK bulgular: K1) `scorer.py::_seviye_trend_skoru` motorunda trend işareti sıfırı geçtiğinde ~5,3 puanlık skor uçurumu (4 mercek spec'i de bu motoru genişletiyor); K2) NCAV/net-net formülü sıfıra/negatife bölünüyor (BİST'te %52,1, NASDAQ'ta %71,4 şirkette payda≤0); K3) "Toplam Yükümlülük/Özkaynak" negatif özkaynakta işaret tersine dönüyor (BİST'te 4/167 şirket, "GÜÇLÜ" bandına yanlış düşüyor) + spec içi çelişki (ağırlıklı mı ağırlıksız mı); K4) x-katı oranların yüzde-formatlayıcıya yanlış beslenme riski; K5) Marjinal ROE payda instabilitesi + tanımsız eşik.
- `scripts/kalibrasyon_v2.py` yazıldı ve DB'deki GERÇEK örneklem (202 ticker — 180 BİST + 22 NASDAQ, `FinancialPeriod` verisi olanlar; toplam Company tablosu 643 BİST + ~4352 NASDAQ ama çoğunda henüz mali veri YOK) üzerinde çalıştırıldı (`--with-price` ile 165 BİST hissesi için canlı F/K/PD/DD dahil, ~7 dk). Bulgu: mevcut v1 eşikleri (`fk_ucuz=8`, `pddd_ucuz=1`) BİST örnekleminin **%55-60'ını "ucuz" bandına düşürüyor** — güncellik sorgulanmalı (spec'e not olarak işlendi, eşik burada DEĞİŞTİRİLMEDİ).
- Bir turda bağlantı hatasıyla yarıda kesildi, transcript'ten devam ettirilerek tamamlandı — token maliyeti bu yüzden normalden yüksek oldu (~300K token bu ajan için).

**2) `temel-analiz-uzmani` ajanı — veri tamlığı çapraz doğrulaması:**
- `docs/spec/veri_tamlik_notu.md` yazıldı: 5 spec'teki ~16 ana "erişilemiyor/veri yok/desteklenmiyor" bayrağı `bilgi-bankasi/00_sentez.md` §3-4 VE doğrudan kod okumasıyla (`calculator.py`, `valuation.py`, `merton.py`, `trends.py`, `isyatirim.py`, `kap_financials.py`, `sec_edgar.py`) çapraz doğrulandı — kitap PDF'leri YENİDEN OKUNMADI, sadece 00_sentez.md kaynak alındı (kullanıcı talimatı, token tasarrufu).
- Sonuç: **7 (A) yanlış-eksik** (veri/hesaplama aslında mevcut — örn. `merton.py::compute_merton_dd_edf()` hâlâ hiçbir modüle bağlı değil, kod üzerinden tekrar doğrulandı; Büyüme merceğinde banka "VERİ EKSİK" yer tutucusu da yanlıştı, `loans`/`deposits` serisi zaten mevcut), **8 (B) ucuz-açılabilir** (somut KAP/SEC XBRL etiketi önerisiyle), **6 (C) gerçek-bloker** (yeni mimari/veri sınıfı gerektiriyor, olduğu gibi bırakıldı).
- **Yeni bulgu (00_sentez.md'de yoktu):** NASDAQ/BİST maliyet asimetrisi — SEC XBRL (`us-gaap:*` etiketleri, örn. `TreasuryStockValue`, `ResearchAndDevelopmentExpense`) NASDAQ tarafında SG&A/Ar-Ge/Faiz Gideri/Hazine Hissesi için ORTA-DÜŞÜK maliyetle açılabilirken, BİST (KAP XBRL) tarafında aynı kalemler hâlâ araştırma gerektiriyor.

**3) Kritik bulguların spec'lere işlenmesi:** `temel-analiz-uzmani` aynı oturumda devam ettirilerek K1-K5 (KRİTİK) + Y1/Y4 (önerilen YÜKSEK) + veri_tamlik_notu.md'nin (A)/(B) kategorileri 5 spec dosyasına FİİLEN işlendi (guard'lar, birim uyarıları, formül netleştirmeleri, spec-içi çelişki çözümleri — spec_mercek_guvenlik.md'deki Toplam Yükümlülük/Özkaynak ağırlık çelişkisi gibi). ORTA/DÜŞÜK bulgular (O1-O4, D1-D3) BİLİNÇLİ olarak işlenmedi (kapsam/token disiplini — kod yazılıp gerçek korelasyon ölçülünce ele alınabilir, blocker değil).

**Commit:** `7d4675c` — `docs(spec): Faz 3a quant denetimi + kalibrasyon_v2 + KRITIK duzeltmeler` (8 dosya, +1417/-62), push edildi (`origin/main`, github.com/serefsametumutlu/QuaxisLabs).

**SONUÇ: Spec'ler kodlamaya HAZIR onayı verildi.** Kalan (C) gerçek-bloker veri kalemleri (10 yıllık seri, harici kredi notu, BİST faiz gideri/SG&A/Ar-Ge) 00_sentez §4 referanslarıyla yol haritasında kaldı, bu Faz'ın kapsamı dışında.

### Sıradaki adım — YENİ OTURUMDA BAŞLANACAK (kullanıcı limit nedeniyle durdurdu, 2026-08-11)
**FAZ 3b (kodlama):** `kod-gelistirici` agent'ı ile 5 spec'i (spec_mercek_deger/kalite/buyume/guvenlik.md + spec_bilesik_skor.md, artık KRİTİK düzeltmeleriyle güncel — commit `7d4675c`) Python koduna dökmek — mevcut 1211 testi kırmadan, `src/analysis/scorer.py`'nin `_seviye_trend_skoru` motorundaki cliff düzeltmesini (K1, TEK yerde/çekirdekte) ÖNCE yapıp sonra 4 mercek + bileşik skor modüllerini eklemek mantıklı bir sıra olabilir. HENÜZ BAŞLANMADI, hiçbir kod dosyası (src/analysis/ altında yeni mercek modülü) yazılmadı. Ayrıca isteğe bağlı: ORTA/DÜŞÜK bulgular (quant_denetim_01.md O1-O4/D1-D3) kodlama sırasında/sonrasında gözden geçirilebilir.
