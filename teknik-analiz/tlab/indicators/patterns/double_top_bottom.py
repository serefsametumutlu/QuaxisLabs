    ...
    def compute(self, df: pd.DataFrame, context: dict | None = None) -> IndicatorResult:
        ...
        for kind, pattern_name in (("high", "double_top"), ("low", "double_bottom")):
            ...
            for p1, neckline_pivot, p2 in _matched_pairs(zigzag, kind):
                ...
                # `entry_sig`, `confirm_sig` DEĞİL (yukarıdaki `confirm_sig`
                # hacim-onaysızsa `pattern_signals`'tan ÇIKARILMIŞ olabilir --
                # `confirm_signal()` GÜNCEL listeden yeniden arar). AL/SAT ve
                # hedef Level'i yalnızca GERÇEKTEN onaylanmış bir kırılım
                # varsa üretilir; formasyon geçersizleşmiş/süresi dolmuşsa da
                # HİÇBİRİ üretilmez (eski bir kırılımın giriş işareti kalıcı
                # bir "AL" olarak asılı kalmasın diye).
                ...
                # Genel durum rozeti -- formasyonun GÜNCEL durumu, last_sig
                # barında (bu doğru davranış, DEĞİŞTİRİLMEDİ).
                markers.append(
                    Marker(
                ...
                # K3 düzeltmesi (2026-09-05): DÖRT AYRI işaret --
                # KIRILIM/ONAY/AL-SAT/HEDEF. AL/SAT artık last_sig'e DEĞİL,
                # entry_sig (kırılım onayı) barına konur; formasyon geçersiz/
                # süresi dolmuşsa hiçbiri üretilmez.
                if show_entry:
                    assert entry_sig is not None
                    entry_price = close[df.index.get_loc(entry_sig.bar_time)]
                    markers.append(
                        Marker(
                            t=entry_sig.bar_time, price=entry_price, text="KIRILIM",
