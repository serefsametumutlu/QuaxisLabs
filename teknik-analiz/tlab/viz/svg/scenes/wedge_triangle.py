"""`patterns.wedge` / `patterns.triangle` -> saf SVG sahne (Faz 4b, 1/6).

Referans: `docs/design/grafik_stil_vitrini_files/saved_resource.html::
sceneWedgeTriangle` (satır 923-993) -- 4 köşeli hologram (üst+alt sınır
çizgisinin uçları, kendi kendini kesmeyen tek bir yamuk) + iki sınır
çizgisi + "Kırılım"/"Onay: Test Tuttu" işaretleri + durum rozeti + hedef
çizgisi/etiketi. `double_top_bottom.py`nin AYNI desenini izler (grup ->
en güncel long/short -> twoUp, `resolve_collisions`e verilen tek etiket
havuzu) -- BİR farkla: burada tek bir "boyun" YOK, iki BAĞIMSIZ sınır
çizgisi var (`_upper`/`_lower`, `WedgeIndicator`/`BroadeningIndicator`nin
PAYLAŞTIĞI `pattern_boundary` stili) ve kırılan çizgi (`break_line`) yönle
BELİRLENİYOR (`last_state[pid]["direction"]`).

`patterns.wedge` (falling/rising) ve `patterns.triangle` (sym/asc/desc)
AYNI `WedgeIndicator` sınıfından, AYNI `IndicatorResult` sözleşmesiyle
üretilir (yalnızca `mode` parametresi farklı) -- `harmonic.*`nin 8 ekolü
TEK sahneyi paylaşmasıyla AYNI ilke, burada da TEK modül iki CATALOG adına
kayıtlıdır (bkz. `tlab/viz/svg/__init__.py`)."""
...
def _line_value_at(line: Line, df: pd.DataFrame, t: pd.Timestamp) -> float:
    """`Trendline.value_at`in (slope*idx+intercept) AYNI formülü -- sahnede
    `Trendline` nesnesinin kendisi YOK, yalnızca iki uç noktalı `Line`,
    bu yüzden eğim buradan yeniden türetilir (uydurma değer DEĞİL, aynı
    matematiğin tekrarı)."""
...
def _group_patterns(result: IndicatorResult) -> dict[str, _PatternGroup]:
    ...
    # 1. iterasyonda GERÇEK bir hata bulundu: `target.start` formasyonun
    # DOĞUM barıdır (created_idx), "OLUŞUYOR" (pending) durumunda `target.
    # end` de None olduğu için `last_time` yanlışlıkla doğum barına
    # düşüyordu -- hedef etiketi/çizgisi bu ESKİ konuma çizilip panelin
    # SAĞ kenarını taşıyordu. Doğrusu `double_top_bottom.py`deki AYNI
    # yaklaşım: pattern_id'nin GERÇEK en son sinyalinin bar_time'ı.
    ...
    for pid, info in result.last_state.items():
        # `pid`, wedge'de `{mode}_..._{direction}` biçiminde -- ama gerçek
        # sınır çizgisi/hologram anahtarı `direction` EKİ OLMADAN, ortak
        # pattern_key'e göre saklanıyor (iki yön AYNI çizgi çiftini paylaşır,
        # bkz. wedge.py::compute -- upper/lower/hologram pattern_key'e,
        # target/state pattern_id'ye bağlı).
...
def _pick_x_ticks(window: pd.DataFrame) -> list[tuple[float, str]]:
    """`double_top_bottom.py`nin `"%b"` (yılsız) formatının AYNISI DEĞİL --
    1. iterasyonda GERÇEK bir hata bulundu: o sahnenin penceresi birkaç ay
    ile sınırlıyken, burada `wedge`/`triangle`in doğum-hedef aralığı AYLARCA
    (TUCLK'ta 1 yıldan fazla, bkz. CLAUDE.md'nin bilinen "18 ay" notu)
    sürebiliyor -- yılsız "Eyl...Tem" etiketleri hangi Eylül/Temmuz olduğunu
    belirsiz bırakıyordu. `report.py`/`swing_fib_abcd.py`nin AYNI `"%b '%y"`
    biçimi kullanıldı."""
    n = len(window)
    if n < 2:
        return []
...
def _panel_svg(
    ...
    # `broadening.py`de (2. iterasyon) GERÇEK bir hata bulundu: "ölçülü
    # hareket" hedefi negatif çıkabiliyor -- AYNI matematik burada da var
    # (break_line ± height), aynı korumayla tutarlı tutuldu.
    ...
    add(state_text, last_x, chart.y(target_price), 190, 20, 6, _LabelStyle(fill_attr, kind="pill"))
...
def build(result: IndicatorResult, df: pd.DataFrame, theme: SVGTheme) -> SceneOut:
    ...
    scene_label = "Takoz / Üçgen"

    if long_g is None and short_g is None:
        panel = _empty_panel(theme, "Takoz/üçgen adayı yok")
        return SceneOut(
    ...
    if long_g is not None:
        ...
        svg = _panel_svg(result, window, long_g, theme, "gTa", df)
    ...
    if short_g is not None:
        ...
        svg = _panel_svg(result, window, short_g, theme, "gTb", df)
    ...
    return SceneOut(
        ...
        subtitle="İki yakınsayan sınır çizgisi + 4 köşeli hologram", badge=None, two_up=two_up,
    )
